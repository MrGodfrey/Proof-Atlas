After applying [[main.claim.partial_null_control]] on $(a_j,a_j+T_j)$ and [[main.claim.free_decay]] on $(a_j+T_j,a_{j+1})$, one obtains the local one-step estimate
...
The elementary product computations used to simplify the exponent are [[main.calculation.lr_exponent_identities]]. Combining those identities with

$$
j^{\mathcal{M}}=\max\{j\mid 2^{2j}\leq C_1/h^2\},
$$

and using $\rho\in(0,1)$, choose $h_0\leq h_2$ and $C_6>0$ such that for $0<h\leq h_0$,
...
This proves [[main.calculation.lr_product_estimate]].
