...
resolver 根据这个配方计算 proof route，并解释每个对象为什么被纳入。它要回答的问题包括：
...
所以 Generated View 里的 Proof Tree 是 resolved route 的只读投影。树形视图帮助理解证明支撑结构，但不是编辑器。

## 内容充分优先

Proof Atlas 的云端导出目标不是把 token 压到最小，而是先保证上下文充分、来源可追溯、内部链接可读。

在 `proof` route 中，hard dependency 不能只剩 `reference`。它至少需要可读的 `statement`，否则云端 AI 看到的是一串对象名，而不是可验证的数学材料。

当前 UI 不展示 token estimate 或 marginal cost；上下文是否足够由 route status、representation 和 diagnostics 解释。
