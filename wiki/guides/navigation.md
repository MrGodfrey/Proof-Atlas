...
标签。Markdown view 是作者手工组织的阅读入口；Generated View 是从 route 配方解析出的依赖 slice。
...
根据目标对象、proof choice、boundary 和 representation 计算出的依赖 slice。
...
这行说明当前生成视图的目标、route profile、纳入对象数量和当前表示粒度下的 token
估算。token 估算用于理解上下文成本，不代表数学复杂度。
...
Generated View 有两个只读投影：

| Tab | 作用 |
|---|---|
| `Linear` | 把同一个依赖 slice 线性化成阅读顺序。适合人读和检查 proof route。 |
| `Graph` | 把同一个依赖 slice 画成图。适合看依赖结构、witness path 和 boundary。 |

Graph 里的控件：

| 控件 | 作用 |
|---|---|
| 搜索框 | 按对象 name、title、role、decision、representation 过滤高亮节点。 |
| `-` / `+` | 缩小或放大图。 |
| 滚动画布 | 移动图的位置。Back 返回时会恢复图的 tab、搜索、缩放和位置。 |
| 单击节点 | 只更新右栏，展示该对象为什么被 route 纳入。 |
| 双击节点 | 进入对象完整页。 |
| 单击边列表 | 选中边的目标对象，右栏显示对象详情。 |

Generated View 中的图不是编辑器。搜索、缩放、高亮和点击都不会改变 route 文件。
...
完整页顶部的 `Paper view` 返回按钮会回到当前 manual view。浏览器 Back 会回到上一个实际历史位置。
