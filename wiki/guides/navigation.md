...
这行说明当前生成视图的目标和 route 是否闭合。紧凑摘要条还显示结构状态、
上下文充分性、proof choice 模式、verification 计数、accepted input、context cut
和 open blocker。
...
Generated View 的只读 tab 按阅读优先级排列：

| Tab | 作用 |
|---|---|
| `Proof Tree` | 默认视图。先显示 target claim 和 selected proof，再由用户展开 proof 的直接 uses。 |
| `Narrative` | 只显示与当前 proof route 实际纳入对象相关的 note 正文；没有 note 时显示空态，不把 target 的所有 `related_to` 内容都塞进来。 |
| `Proof Choices` | 只有存在可替换 proof 时才出现。它显示当前 selected proof，并把可选替代 proof 标成可点击选项；没有 alternatives 的 claim 不占用这个 tab 的空间。 |
...
| `Expand main path` | 按当前树的展示顺序深度遍历并展开主路径；同一个对象如果在上文已经展开过，后续重复出现的位置保持收起，避免同一张卡片被自动展开多次。 |
...
| `boundary type` | boundary 是 external/imported accepted input 还是内部 context cut。 |
