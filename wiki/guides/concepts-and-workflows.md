...
`views/*.route.yml` 是 Generated View：保存目标、proof choice、boundary 和表示粒度，由 resolver 计算出依赖 slice。

Generated View 的图和线性列表都是输出，不是编辑器。浏览器里点图、搜索、缩放都不会改文件。
...
2. 先看 `Linear`：按依赖顺序读 definitions、models、supporting claims、proofs 和 target。
3. 再切到 `Graph`：看依赖结构和 boundary。
4. 单击某个节点，在右栏查看 `Route inclusion`。
5. 重点看这些字段：
...
| `diagnostics` | 是否有 proof choice、statement 或 profile 问题。 |
...
浏览器中的 `Graph` 只是帮助阅读这条 route；如果要改变 proof choice、boundary 或 representation，改 route 文件或用 CLI。
...
- hard dependency 在 proof / meaning context 中不能只给 reference。
