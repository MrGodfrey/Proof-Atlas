...
- `src/core/routeResolver.ts`：proof 展开规则、proof 选择、boundary、representation 下限、route status。
...
定理、引理、命题、推论、猜想和普通 claim 可以作为根节点。`proof`、`definition`、
`setting`、`notation`、`assumption`、`problem`、`note` 和 `issue` 不能作为 Generated View 根节点。
...
`setting`、`definition`、`notation`、`assumption` 等材料不混入主树；网页在中栏主树下方按进入关系分成单列 context 组，例如 `Required Context`、`Used Inputs` 和 `Citation and Source Context`。右栏关系区仍可查看同一对象的完整出边和反向边。
...
`boundaries` 表示“在这里停止展开 outgoing hard dependencies”。resolver 和导出层会把 boundary 分成两类：

- `Accepted inputs`：外部或导入的 math claim boundary。
- `Context cuts`：内部 setting、definition、notation、assumption、proof 或其他 support boundary。
...
只有从 target 解析路径实际遇到的 boundary 会进入 route 节点列表。显式写了但没有被遇到的 boundary 会产生 `unused_boundary` warning，`route --save` 不会把它写回 frozen route。

Markdown 导出会区分 accepted input 和 context cut：
...
|---|---|---|---|
| `full` | 导出对象 `body` 列表中的所有 Markdown 文件。 | target、selected proof、必须检查完整推理的 proof 或 definition。 | `main.proof.lr_iteration: full`。 |
...
| `omit` | 不导出正文。 | 只用于 soft context 中你明确不想放入的对象。 | 把大型旁支 note 从 context 中拿掉。 |
...
- `setting` / `notation` / `definition`
  没有 `statement.md` 时，可使用第一个 body 文件作为 `statement`。
- 其他对象，包括 `claim`、`problem`、`assumption`、`proof`
...
`npm run atlas -- route` 输出包括 route status、对象数量、selected proofs、实际遇到的 boundaries、open blockers、diagnostics，以及每个节点的 witness path。route status 汇总 structure、context、proof choice、verification、accepted inputs、context cuts 和 open blockers。
...
8. `proof_choices` 只能选择 `role: proof` 且确实 `proves` 对应 claim 的对象。
9. 修改后运行 `npm run atlas -- check --strict <paper-root-or-ProofAtlas-root>`。
...
- 在顶部输出一行轻量 `Route status`，但不输出完整 diagnostics、certificate 或大段 YAML metadata。
- 输出 Proof Route、Accepted Inputs、必要时的 Context Cuts、Target、Definitions and Settings、Supporting Claims、Proofs、Issues、Citation / Source Notes 和 References。
...
- Accepted Inputs 来自 resolved route 中 `decision: boundary` 且对象是外部/导入 math claim 的节点，包括显式 `boundaries` 和外部 claim 无展开 proof 时产生的隐式 boundary。内部 boundary 显示为 Context Cuts。
